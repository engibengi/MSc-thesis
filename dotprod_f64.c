...
extern "C" double* dotprod(double *A, double *B, double *C);
...
int main() {
  ...
  double *local_B = local_A + EL;
  double *local_C = local_B + EL;
  double *local_D = local_C + 1;   
  ...
  if (snrt_is_dm_core()) {
    snrt_dma_start_1d(local_A, (volatile void *)A, EL * sizeof(double));
    snrt_dma_start_1d(local_B, (volatile void *)B, EL * sizeof(double));
    snrt_dma_start_1d(local_C, (volatile void *)C,  1 * sizeof(double));
    snrt_dma_start_1d(local_D, (volatile void *)D,  1 * sizeof(double));
    snrt_dma_wait_all();
  }
