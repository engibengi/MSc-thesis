...
cp ./compile-and-run-nocanon.sh $REPO
...
docker run --rm -ti --volume $REPO:/src ghcr.io/opencompl/snitch-toolchain:4.0.0 /bin/bash -c "cd /src; ./compile-and-run-nocanon.sh $MAINC $KERNELMLIR $COMPILED"
